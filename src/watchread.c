...
static void
button_slower(void *data, Evas_Object *obj, void *event_info )
{
	USER_SPEED -= 20;
	if (USER_SPEED <= 100)
	{
...
static void
button_fast(void *data, Evas_Object *obj, void *event_info )
{
	USER_SPEED += 20;
}

static void
button_back(void *data, Evas_Object *obj, void *event_info )
{
	flag = 0;
}
...
create_base_gui(appdata_s *ad, int width, int height)
{
	...
	evas_object_move(ad->faster, 4 * width / 5, height / 2 - 40);
	...
	evas_object_move(ad->slower, 10, height / 2 - 40);
	...
	evas_object_move(ad->back, 0, height / 3 + height / 4);
